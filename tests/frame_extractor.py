...
def imageSender(out_frame_path, server, numpy_bytes):

    with RabbitMq(server) as rabbitmq:
        rabbitmq.publish(payload=numpy_bytes)

def getFrame(sec, count, vidcap, server):
    ...
    if hasFrames:
        ...
        out_frame_path = f'images_in/{str(count)}.jpg'
        ...
        if out_frame_path:
            imageSender(out_frame_path, server, im64_bytes)
...
def frame_extractor(video_path):
    ...
    frameRate = 0.5
    count=1
    success, out_frame_path = getFrame(sec, count, vidcap, server)
